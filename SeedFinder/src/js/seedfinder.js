let seeds = [];
    ...
    async function loadSeeds() {
      try {
        ...
        renderSeeds(seeds);
        buildOptions('seedbankFilter', seeds.map(s => s.seedbank));
        buildOptions('geneticFilter', seeds.map(s => s.genetics));
        buildOptions('flowerFilter', seeds.map(s => s.flowering_time));
        buildOptions('yieldFilter', seeds.map(s => s.indoor_yield));
      } catch (err) {
    ...
    function toggleViewFavs() {
      showFavs = !showFavs;
      document.getElementById('favToggle').textContent = showFavs ? "Alle anzeigen" : "Nur Favoriten";
      filterSeeds();
    }
    ...
    function filterSeeds() {
      ...
      const result = seeds.filter(s =>
        ...
        s.thc >= minThc && s.thc <= maxThc &&
        s.cbd >= minCbd && s.cbd <= maxCbd &&
