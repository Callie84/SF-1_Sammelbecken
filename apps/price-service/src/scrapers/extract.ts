...
function filterItems(items: Item[]): Item[] {
  const BAD_NAME =
    /^(home|filter results|filters?|sort(?: by)?|subscribe|contact|read more|add to cart|cart|wishlist|login|logout|account|menu|shop now|view more)$/i;
  return items
...
function fromJsonLd($: cheerio.CheerioAPI): Item[] {
  const pushProduct = (p: any, out: Item[]) => {
    ...
    const priceRaw = String(
      offers?.price ?? offers?.priceSpecification?.price ?? p?.price ?? ""
    );
    const curRaw = String(
    ...
    const { price, currency } = priceRaw
      ? parsePrice(`${priceRaw} ${curRaw}`)
      : { price: null, currency: curRaw || null };
...
export default function extract(html: string): Item[] {
  ...
  const $ = $root as unknown as cheerio.CheerioAPI; // <- wichtig für TS
  ...
  if (items.length === 0) {
    ...
    nodes.each((_, el) => {
      ...
      let name =
        el$
          .find(
            "h1,h2,h3,.product-title,.card-title,.title,.product-name,.product-item__title,.grid-product__title,.card-information__text,.product-card__title,a.full-unstyled-link,a[title]"
          )
          .first()
          .attr("title") ||
        el$
          .find(
            "h1,h2,h3,.product-title,.card-title,.title,.product-name,.product-item__title,.grid-product__title,.card-information__text,.product-card__title,a.full-unstyled-link,a"
          )
          .first()
          .text() ||
        "";
    ...
    if (out.length === 0) {
      ...
      $("*").each((_, el) => {
        ...
        const box = $(el).closest(
          "li,article,div,section,.card,.product,.product-item,.product-card,.productgrid--item,.grid__item"
        );
        ...
        const badName2 =
          /^(home|filter results|filters?|sort(?: by)?|subscribe|contact|read more|add to cart|cart|wishlist|login|logout|account|menu|shop now|view more)$/i;
        if (!name || badName2.test(name)) return;
