...
cron.schedule('0 0 * * *', async () => {
  ...
  const collections = ['users','growlogs','alerts','stats','bookmarks','uploads','seedbanks'];
  for (const col of collections) {
    const csvPath = path.join(__dirname, `../../exports/${col}.csv`);
    await exportCollectionCSV(require(`../models/${col[0].toUpperCase() + col.slice(1)}`), [], csvPath);
  }
