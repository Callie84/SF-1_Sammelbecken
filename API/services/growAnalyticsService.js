...
async function getOverallStats(userId) {
  ...
  const avgDurationDays = totalCycles ? (avgDuration / totalCycles).toFixed(1) : 0;
  return { totalCycles, avgEntries: Number(avgEntries), avgDurationDays: Number(avgDurationDays) };
}
