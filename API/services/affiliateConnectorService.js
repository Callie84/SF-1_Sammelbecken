...
async function fetchSeedExpressOffers(strain) {
  const response = await fetch(`https://api.seedexpress.com/offers?strain=${encodeURIComponent(strain)}`);
...
async function fetchGreenShopOffers(strain) {
  const response = await fetch(`https://api.greenshop.com/v1/seeds?search=${encodeURIComponent(strain)}`, {
