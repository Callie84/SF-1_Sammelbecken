...
async function sendVerificationEmail(user) {
  ...
  await sendEmail(user.email, 'Verify your account', `Click <a href="${link}">here</a> to verify.`);
}

async function sendPasswordResetEmail(user) {
  const token = await createPasswordResetToken(user._id);
  const link = `${process.env.FRONTEND_URL}/reset-password/${token}`;
  await sendEmail(user.email, 'Reset your password', `Click <a href="${link}">here</a> to reset.`);
}
