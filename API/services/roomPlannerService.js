...
async function upsertLayout(userId, data) {
  ...
  let layout;
  if (id) {
    layout = await RoomLayout.findOneAndUpdate({ _id: id, userId }, { name, width, length, placements }, { new: true });
  } else {
...
async function fetchPartnerPrices(itemIds) {
  ...
  return itemIds.map(id => ({ itemId: id, price: (Math.random()*50 + 10).toFixed(2) }));
}
