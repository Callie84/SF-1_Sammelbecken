const { upsertLayout, listLayouts, fetchPartnerPrices } = require('../services/roomPlannerService');

// POST /planner/layout → Layout erstellen oder updaten
exports.saveLayout = async (req, res) => {
  const layout = await upsertLayout(req.user.id, { id: req.body.id, ...req.body });
  res.json(layout);
};
...
exports.getPrices = async (req, res) => {
  const { placements } = await listLayouts(req.user.id).then(ls => ls.find(l => l.id == req.params.id));
