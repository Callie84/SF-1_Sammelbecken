...
exports.reviewUpload = async (req, res) => {
  ...
  // Admin-Aktion loggen
  await logAction(req.user.id, approved ? "UPLOAD_APPROVED" : "UPLOAD_REJECTED", { uploadId: id, feedback });
...
exports.getStatus = async (req, res) => {
  const uploads = await UploadedSeed.find({ userId: req.user.id }).select("strain moderation");
  res.json(uploads);
};